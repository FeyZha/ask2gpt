...
- [ ] `pnpm package` 生成 `ask2gpt-0.1.0.vsix` 与
      `ask2gpt-relay-0.1.0.zip`。
...
- [ ] 安装 `ask2gpt-0.1.0.vsix` 后执行 **Reload / 重新加载**；没有提示时运行
      `Developer: Reload Window`。
- [ ] 在 `chrome://extensions` 中加载 0.1.0 Relay 目录；升级时必须完整替换解压目录，禁止只覆盖
...
- [ ] 点击待发送或已发送上下文卡片主体会打开 Host 保存的 URI 并选中对应行；文件内容整体移动后，
      唯一快照内容仍定位新行，重复内容不得误跳第一个匹配。文件不存在或范围失效时保留卡片并明确提示。
- [ ] 回答普通文本或 inline code 中的 `06_vector_store.py:34`、`:34-40`、`#L34-L40` 显示为
      可键盘操作的源码按钮；点击只在该回答之前明确附加的上下文/附件别名中定位并选中行区间。
      HTTP 链接、代码围栏、路径穿越和未附加文件不得转换或打开。
- [ ] 回答中的已附加函数引用（如 `get_embeddings_endpoint()`）可点击并高亮定义名称；单定义直接
      打开，多定义使用 QuickPick，取消不改变状态，无语言服务时只允许从该轮有界快照回退。
- [ ] 在编辑器选中曾发送的代码后，编辑器标题栏、右键菜单或命令面板的“查找关联对话”可反查轮次：
      单匹配直接定位，多匹配按相关度/时间显示 QuickPick，零匹配可选择“使用此选区提问”。归档对话
      需确认恢复；定位后侧栏滚动到 user turn 并短暂显示“匹配此选区”，不改变编辑器内容。
